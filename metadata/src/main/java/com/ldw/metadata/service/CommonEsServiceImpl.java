...
//            log.info("added jsonEntity to es successfully !  index:{} , id:{} ",index,id);
    ...
    public Boolean update(String index, String id, String jsonEntity) {
        Boolean result = null;
        try {
            UpdateRequest request = new UpdateRequest();
            request.index(index).id(id).doc(jsonEntity, XContentType.JSON)
...
//									.lt(format.format(mapV.get("end"))));
  ...
  /*  @Override
     ...
     * @throws IOException
     */
    private List<JSONObject> scrollSearchAll(String index, BoolQueryBuilder boolBuilder) {
        ...
        try {
            ...
            log.error("scrollSearchAll 异常", e);
        } catch (ElasticsearchStatusException e) {
            if (e.getMessage().contains("index_not_found_exception")) {
                log.error("没有索引，现在创建", e);
                createIndex(index);
            }
        }
        ...
        try {
            ...
            log.error("scrollSearchAll 异常", ioException);
        }
        return resultList;
    }
  ...
  /*  @Override
    public JSONObject findById(String index, String id) {
